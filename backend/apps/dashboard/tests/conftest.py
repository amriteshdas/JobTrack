...
def resume_file():
    """
    Returns a FACTORY, not a single file object -- Django's uploaded-file
    objects are consumed (read to EOF) after one request, so reusing the
    same instance across multiple apply() calls in one test silently sends
    an empty file on the second call. Every call site gets a fresh file.
    """
    def _make():
